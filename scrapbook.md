...
- **Zenodo flow (RDM API):** `POST /api/records/{id}/versions` → new recid; `PUT /draft` metadata (needs `publisher`, `person_or_org.given_name/family_name`); `POST /draft/actions/publish` → 202. **Final active/latest version = recid `22965178`** (v5, PAPER.pdf 78966 bytes). Superseded-but-live: v1 22961527 / v3 22963620 / v4 22964117. File drop = manual drag-and-drop (AGENTS.md rule), not setInputFiles.
- **DOI — FINAL:** cite the **concept DOI `10.5281/zenodo.22961526`** (STABLE, future-proof). Verified it resolves (200) → `zenodo.org/records/22965178` (latest). Paper + README + local PDF + uploaded Zenodo PDF all cite it (consistent). Concept DOI→latest, version DOI 10.5281/zenodo.<recid>→that version.
- **DOI resolution:** doi.org 404 right after publish is *expected* DataCite handle-registration lag (verified vs Zenodo/EPrints docs); a version DOI stays 404 for ~minutes while the handle registers, concept DOI once family is registered. `hdl.handle.net` `responseCode:100` (no URL value yet) = in-progress, not failure.
